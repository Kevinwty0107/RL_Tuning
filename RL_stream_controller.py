...
def eval_policy(policy, data, eval_episodes=5):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_timesteps", default=10, type=int)# Time steps initial random policy is used
    parser.add_argument("--eval_freq", default=30, type=int)       # How often (time steps) we evaluate
    parser.add_argument("--max_timesteps", default=900, type=int)   # Max time steps to run environment
    ...
    parser.add_argument("--env_change_freq",default=20)     # random_local: local + global sampling, random: only global sampling
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            np.save(f"./results/{args.search_method}/KT_{file_name}_{data_name}", evaluations)
            if args.save_model: policy.save(f"./rlmodels/{file_name}")

        if (t + 1) % args.env_change_freq ==0  and change_i <=28:
        ...
        # Train agent after collecting sufficient data
            ...
            if t >= args.start_timesteps:
                if policy.memory_counter > 15:  # type: ignore
